...
def averagetimeseries(dset, dates):
    ...
    for time in dset:
        total = 0
        n = 0
        for lat in time[205:216]:
            for measurement in lat[205:216]:
                total += measurement
                n += 1
        total /= n
        res.append(total)
...
def index():  # put application's code here
    ...
    if request.method == "POST":
        if "latitude" in request.form:
            ...
            plt.figure()
            plt.imshow(disp_idx1_idx2 * dset_mask,
                       cmap="jet", vmin=-0.05, vmax=0.05)
    ...
    # Render the form
    return render_template(
        "index.html", plot_filename=None, snapshot_filename=None, coordinates=""
    )
